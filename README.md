...
It contains the license texts and attributions for libraries and fonts bundled with this project and is intended for human review and distribution alongside the source.

To regenerate the canonical notice file, run the helper script:

```powershell
python scripts/generate_thirdparty_notice.py
```

(Alternatively, you can use the Gradle merge task to write generated resources into `app/src/main/res/raw/` with `-PwriteThirdPartyRes=true` and then run the script.)
