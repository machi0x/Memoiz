...
<!-- Note and links to the generated license files in the repository -->
You can find the third-party license texts generated by the Gradle OSS Licenses plugin in this repository under the `build_license_refs/` directory:

- Full concatenated license text: [build_license_refs/third_party/third_party_licenses.txt](build_license_refs/third_party/third_party_licenses.txt)
- Per-component license files: [build_license_refs/third_party/components/](build_license_refs/third_party/components/)
- Extracted font license files: [build_license_refs/fonts/](build_license_refs/fonts/)

For a human-readable summary and notices, see [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
...
```powershell
# Run the merge task and write the generated license resources into src/main/res/raw
.
\gradlew.bat :app:mergeThirdPartyLicenses -PwriteThirdPartyRes=true
```

If you don't need to track them, no action is required; the app will still display OSS licenses at runtime using the generated resources created during build.
